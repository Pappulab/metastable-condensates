...
class Fibril_Optimization:
    ...
    def plotter(self, exp_fibril_list, comp_fibril_list):
        ...
        file = open('exp_fibril_D262V_50_Day_5.txt','w') # Change this
        for item in exp_fibril_list:
            file.write(str(item)+"\n")
        file = open('comp_fibril_D262V_50_Day_5.txt','w') # Change this
    ...
    def fibril_maker(self, free_parameter_list, fixed_parameter_list, csat, plot_check = False):
        ...
        for conc_index, conc_val in enumerate(self.conc_list):
            if conc_val < 49 or conc_val > 51: # Change this
                continue
            else:
...
plot_title = 'D262V_50_Day5_plot_through_501'
...
#starting guess
if plot_save==0:
    with open('best_guess_D262V_40_Day_5.txt') as myfile: # previous concentrations best guess, Change this
    #with open('initial_guess_D262V_50_Day_5.txt') as myfile: # previous concentrations best guess, Change this
        for line in myfile:
            line = line.strip()
            line = line.split(",")
elif plot_save==1:
    with open('best_guess_D262V_50_Day_5.txt') as myfile: # current best fit used to plot, Change this
        for line in myfile:
            line = line.strip()
            line = line.split(",")
...
# cur_opt.fit.fun = 235841.1923257854
...
import csv
if plot_save==0:
    with open('best_guess_D262V_50_Day_5.txt','w') as myoutfile: # Change this
