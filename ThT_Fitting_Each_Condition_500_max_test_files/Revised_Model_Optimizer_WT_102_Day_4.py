...
class Fibril_Optimization:
    ...
    def plotter(self, exp_fibril_list, comp_fibril_list):
        ...
        file = open('exp_fibril_WT_102_Day_4.txt','w') #Change this
        for item in exp_fibril_list:
            file.write(str(item)+"\n")
        file = open('comp_fibril_WT_102_Day_4.txt','w') #Change this
    ...
    def fibril_maker(self, free_parameter_list, fixed_parameter_list, csat, plot_check = False):
        ...
        for conc_index, conc_val in enumerate(self.conc_list):
            if conc_val < 101 or conc_val > 103: # Change this
                continue
            else:
...
# using thtfit_env         
file_name = '/project/fava/work/kiersten.ruff/Collaborations/Mittag/2023/ThT_fitting/For_Kiersten/Revised_Eq_Data/Datasets for fitting/Day 4/WT_data.txt'
...
plot_title = 'WT_102_Day4_plot_through_501'
...
#starting guess
if plot_save==0:
    #with open('best_guess_WT_81_Day_4.txt') as myfile: # previous concentrations best guess, Change this
    with open('initial_guess_WT_102_Day_4.txt') as myfile: # previous concentrations best guess ran false so starting from end of that run, Change this
        for line in myfile:
            line = line.strip()
            line = line.split(",")
elif plot_save==1:
    with open('best_guess_WT_102_Day_4.txt') as myfile: # current best fit used to plot, Change this
        for line in myfile:
            line = line.strip()
            line = line.split(",")
...
# cur_opt.fit.fun = 7126605.749768654 - update this
...
import csv
if plot_save==0:
    with open('best_guess_WT_102_Day_4.txt','w') as myoutfile: # Change this
