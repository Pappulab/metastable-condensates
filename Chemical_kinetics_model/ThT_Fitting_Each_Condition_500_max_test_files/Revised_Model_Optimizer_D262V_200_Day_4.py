...
class Fibril_Optimization:
    ...
    def ax_filler(self, exp_fibril_list, comp_fibril_list):
        ...
        ax.errorbar(x = self.data_time_list[0:(len(exp_fibril_list))], y = exp_fibril_list,
                    fmt = '-', color=Settings.color_list[0],
                    linewidth = Settings.LINE_WIDTH,
                    label='Experimental data')
        ax.errorbar(x = self.data_time_list[0:(len(exp_fibril_list))], y = comp_fibril_list[0:len(exp_fibril_list)],
    ...
    def plotter(self, exp_fibril_list, comp_fibril_list):
        ...
        file = open('exp_fibril_D262V_200_Day_4.txt','w')
        for item in exp_fibril_list:
            file.write(str(item)+"\n")
        file = open('comp_fibril_D262V_200_Day_4.txt','w')
    ...
    def fibril_maker(self, free_parameter_list, fixed_parameter_list, csat, plot_check = False):
        ...
        for conc_index, conc_val in enumerate(self.conc_list):
            if conc_val < 199 or conc_val > 201: # Change this
                continue
            else:
...
# using thtfit_env         
file_name = '/project/fava/work/kiersten.ruff/Collaborations/Mittag/2023/ThT_fitting/For_Kiersten/Revised_Eq_Data/Datasets for fitting/Day 4/D262V_data.txt'
...
# best fit - 592604
#initial_guess = [3.38* 10 ** -3, 4.58 * 10 ** -13, 1.21 * 10 ** -9, 7.58 * 10 ** -11, 1.16 * 10 ** 4,
#                 0.0, 0, 'dilute', 0, 0, 4.22 * 10 ** 1]  
...
plot_title = 'D262V_200_Day4_plot_through_501'

# For initial optimization  
#time_cutoff = 30 # use value from plot_exp_ThT - Change this
#plot_check = 0
#plot_save = 0
...
#starting guess
if plot_save==0:
    with open('best_guess_D262V_160_Day_4.txt') as myfile: # previous concentrations best guess, Change this
    #with open('initial_guess_D262V_200_Day_4.txt') as myfile: # previous concentrations best guess, Change this
        for line in myfile:
            line = line.strip()
            line = line.split(",")
elif plot_save==1:
    with open('best_guess_D262V_200_Day_4.txt') as myfile: # current best fit used to plot, Change this
        for line in myfile:
            line = line.strip()
            line = line.split(",")
...
# cur_opt.fit.fun = 544620.6447864554
...
import csv
if plot_save==0:
    with open('best_guess_D262V_200_Day_4.txt','w') as myoutfile: # Change this
